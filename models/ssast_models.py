...
class SSASTModel(nn.Module):
    def __init__(self, label_dim=1,
                 ...
                 input_fdim=128, input_tdim=1024, model_size='base', task='ft_cls',
        ...
        if pretrain_stage == True:
            if load_pretrained_mdl_path != None:
                raise ValueError(
                    'Setting load_pretrained_mdl_path at pretraining stage is useless, pretraining is always from '
                    'scratch, please change it to None.')
            if fstride != fshape or tstride != tshape:
                raise ValueError(
                    'fstride != fshape or tstride != tshape, they must be same at the pretraining stage, patch split '
            ...
            # this is a trick to make state_dict to track pretraining input_fdim and input_tdim and save them by
            # using torch.save
            ...
            # masked patch classification (discriminative objective) layer we use two layers for pretext task,
            # but using a single layer has similar performance. we map the output of transformer (768-dim for base
            # models) to 256-dim patch input space, and then dot product with flattened patch input (also 256-dim) to
            # calculate loss. alternatively, you can map the output of transformer to 768-dim patch embedding space,
            # and dot product with patch embedding. Performance-wise they are similar, but map to 256 space is more
            # efficient.
            ...
            self.unfold = torch.nn.Unfold(kernel_size=(
                fshape, tshape), stride=(fstride, tstride))

            # we use learnable mask embedding (follow the BEIT paper), but using a fixed mask embedding (e.g.,
            # 0) leads to same performance.
            ...
            # print('pretraining patch split stride: frequency={:d}, time={:d}'.format(
            #     fstride, tstride))
            # print('pretraining patch shape: frequency={:d}, time={:d}'.format(
            #     fshape, tshape))
            # print('pretraining patch array dimension: frequency={:d}, time={:d}'.format(
            #     self.p_f_dim, self.p_t_dim))
            # print('pretraining number of patches={:d}'.format(num_patches))

            # the linear patch projection layer, use 1 channel for spectrogram rather than the original 3 channels
            # for RGB images.
        ...
        # use a pretrained models for finetuning
            ...
            try:
                ...
                p_input_fdim, p_input_tdim = sd['module.p_input_fdim'].item(
                ), sd['module.p_input_tdim'].item()
            except:
                raise ValueError(
                    'The model loaded is not from a torch.nn.Dataparallel object. Wrap it with torch.nn.Dataparallel '
                    'and try again.')

            print('[Info] now load a SSL pretrained models from ' +
                  load_pretrained_mdl_path)
            # during pretraining, fstride=fshape and tstride=tshape because no patch overlapping is used here,
            # input_fdim and input_tdim should be that used in pretraining, not that in the fine-tuning. we need to
            # know input_fdim and input_tdim to do positional embedding cut/interpolation. generally it should be
            # better to use same input_fdim during pretraining and finetuning, but input_tdim can be safely different
            ...
            self.v.patch_embed.num_patches = num_patches
            # print(
            #     'fine-tuning patch split stride: frequncey={:d}, time={:d}'.format(fstride, tstride))
            ...
            if fshape != p_fshape or tshape != p_tshape:
                raise ValueError(
                    'The patch shape of pretraining and fine-tuning is not consistant, pretraining: f={:d}, t={:d}, '
                    'finetuning: f={:d}, t={:d}'.format(
                        p_fshape, p_tshape, fshape, tshape))

            # patch split stride generally should be different for pretraining and fine-tuning, as patch split
            # overlapping is only used in finetuning during pretraining, p_fshape = p_fstride and p_tshape = p_tstride
    ...
    def forward(self, x):
        ...
        if self.cls_token_num == 2:
            ...
            # x = (x[:, 0] + x[:, 1]) / 2
            cls_tokens, dist_token, x = torch.split(x, [1, 1, N], dim=1)
            # clip_rep = (cls_tokens + dist_token) / 2
        else:
            # x = x[:, 0]
            cls_tokens, x = torch.split(x, [1, 1, N], dim=1)
            # lip_rep = cls_tokens
        # x = self.mlp_head(x)
        #clip_lable = self.mlp_head(clip_rep).squeeze(1)
        return x
